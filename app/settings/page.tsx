...
import { Settings2, ShieldAlert, HardDrive, Bell, Moon, Sun } from 'lucide-react';
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <header className="mb-8">
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">设置</h1>
      ...
      <div className="space-y-6">
        ...
        <section className="bg-zinc-200/60 dark:bg-zinc-800/60 rounded-2xl p-2 transition-colors">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 space-y-8 transition-colors">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-1">默认目标盘符</label>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-3">新建迁移方案时默认使用的目标基础路径</p>
              <input 
                type="text" 
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100">静默检测软链接状态</h3>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">启动应用时自动扫描是否有失效的 Junction 链接</p>
              </div>
        ...
        <section className="bg-zinc-200/60 dark:bg-zinc-800/60 rounded-2xl p-2 transition-colors">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 space-y-8 transition-colors">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100">系统核心目录黑名单保护</h3>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">禁止迁移 C:\Windows, C:\Program Files 等核心路径，防止系统崩溃</p>
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-3">自定义黑名单路径</label>
              <textarea 
                rows={3}
                ...
                disabled
              />
              <p className="text-[10px] text-amber-600 dark:text-amber-500 mt-2">内置保护规则不可修改</p>
            </div>
          </div>
        </section>
      </div>
